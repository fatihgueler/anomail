-- Zwei Ergaenzungen an den bestehenden Funktionen aus AP5 und AP6.
--
-- 1. Blockierung wirkt beim Weiterschreiben.
--    assign_letter() beruecksichtigt Blockierungen schon seit AP2, das
--    Weiterschreiben im Briefwechsel bisher nicht - dort waere das Blockieren
--    an der Serverseite vorbeigelaufen und haette nur die Oberflaeche
--    versteckt.
--
--    Der Fehlercode AN014 traegt bewusst keine Richtung: die Anwendung meldet
--    beiden Seiten denselben Text. Waere die Meldung fuer die blockierte
--    Person eine andere, koennte sie daran ablesen, dass sie blockiert wurde.
--
-- 2. Die Benachrichtigungspraeferenz wird beachtet.
--    Ein Schalter, der nichts bewirkt, waere eine Zusage ohne Deckung.
...
BEGIN
  ...
  -- Wirkt in beide Richtungen: wer blockiert hat und wer blockiert wurde,
  -- kann gleichermassen nicht mehr schreiben.
  IF app.is_blocked_between(v_sender, v_recipient) THEN
    RAISE EXCEPTION 'Keine neuen Nachrichten in diesem Briefwechsel'
      USING ERRCODE = 'AN014';
  END IF;
  ...
  IF NOT v_hold THEN
    SELECT notifications_enabled INTO v_wants FROM users WHERE id = v_recipient;
...
BEGIN
  ...
  IF NOT v_hold THEN
    SELECT notifications_enabled INTO v_wants
      FROM users WHERE id = v_letter.author_id;
